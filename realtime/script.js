...
function getWeather() {
  ...
  axios
    ...
    .then((response) => {
      ...
      weather__minmax.html(
        `<p>Min: ${data.main.temp_min.toFixed()}&#176</p><p>Max: ${data.main.temp_max.toFixed()}&#176</p>`
      );
...
$(document).ready(function () {
  $("#weather__searchform").autocomplete({
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $.each(data, function (index, item) {
            if (
              item.name.toLowerCase().startsWith(request.term.toLowerCase())
            ) {
              suggestions.push({ value: item.name });
            }
          });
    ...
    select: function (event, ui) {
      // Submit form
      $("#weather__searchform").val(ui.item.value);
      $(".weather__search").submit();
    },
  });
});
